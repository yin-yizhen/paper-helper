...
| 目标 | 主要文件 | 配套测试 | 验证命令 |
| --- | --- | --- | --- |
| 改任务调度、可见 Chrome 与承诺边界 | `SKILL.md`、`references/environment-and-recovery.md` | `tests/run_smoke_tests.ps1` + Chrome 手工验收 | `python ...quick_validate.py .` |
...
```text
用户主题 -> SKILL.md 最少选择 -> 用户可见 Chrome 中的 CNKI 专用 skill / 浏览器核验
...
- CNKI 访问必须在用户可见 Chrome 中进行；`ERR_CERT_*` 时停在当前窗口让用户处理，不能切到后台浏览器或忽略证书错误。
