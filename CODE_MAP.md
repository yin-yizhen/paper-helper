...
| 目标 | 主要文件 | 配套测试 | 验证命令 |
| --- | --- | --- | --- |
| 改任务调度、可见浏览器连接与承诺边界 | `SKILL.md`、`references/environment-and-recovery.md` | `tests/run_smoke_tests.ps1` + Chrome 扩展/应用内浏览器手工验收 | `python ...quick_validate.py .` |
...
```text
用户主题 -> SKILL.md 最少选择 -> 用户可见且已连接的浏览器中的 CNKI 专用 skill / 浏览器核验
