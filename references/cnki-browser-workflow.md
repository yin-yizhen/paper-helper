...
CNKI 操作必须在用户可见的 Chromium 浏览器窗口完成。优先复用用户已打开的 Chrome 与其登录会话；Chrome 不存在或无法控制时，使用 Microsoft Edge。若需要新开窗口，按 Chrome、Edge 的顺序尝试并将窗口带到前台。不得用后台、无头或不可见浏览器会话代替。
...
出现登录、权限、验证码或 `ERR_CERT_*` 证书错误界面时，保持当前浏览器窗口停在页面并请求用户操作；不得使用非页面授权的下载路径、忽略证书错误或规避措施。若当前环境不能显示/控制 Chrome 或 Edge，停止 CNKI 核验并如实标记为待核验。
